...
pub fn processCapabilityResponse(self: *Terminal, response: []const u8) void {
    ...
    // Kitty detection
    if (std.mem.find(u8, response, "kitty")) |_| {
    ...
    if (std.mem.find(u8, response, ";c")) |pos| {
        var start: usize = 0;
        if (pos >= 4) {
            start = pos;
            ...
            if (std.mem.find(u8, da_response, "\x1b[?") == 0) {
                if (std.mem.find(u8, da_response, "4;") != null or std.mem.find(u8, da_response, ";4;") != null or std.mem.find(u8, da_response, ";4c") != null) {
                    self.caps.sixel = true;
                }
            }
        }
    ...
    if (std.mem.find(u8, response, "\x1b_G")) |_| {
        if (std.mem.find(u8, response, "i=31337")) |_| {
